...
function openApp(id){$("#start-menu").hidden=true;({computer:openComputer,browser:openBrowser,projector:openProjector,mail:openMail,radio:openRadio,notes:openNotes,trash:openTrash}[id]||(()=>{}))()}
...
function bindPage(page,root,w){
  ...
  if(page==="albumLogin")$("#album-submit",root).onclick=()=>{FogLogic.matches("album",$("#album-pass",root).value)?renderPage("album",w):$("#album-error",root).textContent="口令错误"};
  if(page==="labLogin")$("#lab-submit",root).onclick=()=>{FogLogic.matches("lab",$("#lab-pass",root).value)?renderPage("lab",w):$("#lab-error",root).textContent="找不到关联项目"};
...
function openCaseArchive(){
  ...
  $("#case-open",w).onclick=()=>{
    if(!FogLogic.matches("case98",$("#case-pass",w).value)){$("#case-error",w).textContent="箱号或件数不符。";return}
...
function openIncidentReview(){
  ...
  $("#review-submit",w).onclick=()=>{
    ...
    if(!FogLogic.reviewMatches(values)){$("#review-error",w).textContent="复核未通过。回执未生成。";return}
...
function openSwArchive(){
  ...
  $("#sw-open",w).onclick=()=>{
    if(!FogLogic.matches("sw",$("#sw-pass",w).value)){$("#sw-error",w).textContent="密码错误。";return}
    if(!state.chainRevisited){$("#sw-error",w).textContent="口令校验通过，但缺少摘要要求的邮件回看记录。";return}
...
function openSwFile(id){
  if(id==="letter"){
    const required=["argument","interviews","bbs-cache","ticket"];
    if(required.some(file=>!state.swFiles.includes(file)))return showDialog("这封信的修改时间最晚。先把同目录里较早的四份材料看完。")
    const w=createWindow("sw-letter","给梁真.txt - 记事本",pages.ending(),{width:"660px",height:"560px",code:"TXT"});
    $("#end-reset",w).onclick=resetGame;
    return;
  }
  if(!state.swFiles.includes(id)){state.swFiles.push(id);save()}
...
function openProjector(){
  const w=createWindow("projector","投射仪 - SIDE A",`<div class="projection-app"><div class="projection-stage"><div class="projection-noise"></div><p>NO CARTRIDGE / 光路未校准</p></div><aside class="projection-console"><b>PROJECTOR K-1647</b><p>槽里卡着一张没有厂名的投射卡。背面蓝笔：</p><blockquote>先用订购单第一盏灯的绿线校光。别按“绿色”猜，填波长。</blockquote><label>光谱线 <input id="projection-wave" type="number" min="380" max="700" step="0.1" value="500"> nm</label><input id="projection-slider" type="range" min="380" max="700" step="0.1" value="500"><button class="projection-button" id="projection-expose">曝光</button><p class="projection-status" id="projection-status">灯温正常。画面未锁定。</p></aside></div>`,{width:"760px",height:"560px",code:"SIDE-A"});
  const wave=$("#projection-wave",w),slider=$("#projection-slider",w);wave.oninput=()=>slider.value=wave.value;slider.oninput=()=>wave.value=slider.value;
  $("#projection-expose",w).onclick=()=>{if(!FogLogic.projectionMatches(wave.value)){$("#projection-status",w).textContent="画面偏色。卡槽没有响应。";return}renderProjectionCarrier(w)};
}
function renderProjectionCarrier(w){
  $(".window-body",w).innerHTML=`<div class="projection-app carrier"><div class="projection-stage"><img src="https://kuthorx.github.io/images/story_of_cloak/%E6%96%97%E7%AF%B7_A.webp" alt="戴斗篷的猫科形态投影"><div class="projection-caption">K-1647 / FRAME UNSTABLE</div></div><aside class="projection-console"><b>投射卡接通</b><div class="projection-log"><p>“先别拔线。这里也在找这台机器。”</p><p>“有个叫耳鱼的听见了载波。他说调到自己的登记号，四位数。”</p><p>卡片边缘压印：<strong>A-4597 / EarFish</strong></p></div><form id="carrier-form"><label>载波 <input id="carrier-frequency" inputmode="numeric" maxlength="4"> Hz</label><button class="projection-button">接收</button></form><p class="projection-status" id="carrier-status">等待载波。</p></aside></div>`;
  $("#carrier-form",w).onsubmit=e=>{e.preventDefault();if(!FogLogic.matches("carrier",$("#carrier-frequency",w).value)){$("#carrier-status",w).textContent="只有电流声。";return}renderProjectionRecord(w)};
}
function renderProjectionRecord(w){
  $(".window-body",w).innerHTML=`<div class="projection-app record"><div class="projection-stage"><div class="earfish-signal"><span></span><span></span><span></span><span></span><span></span></div><div class="projection-caption">A-4597 / AUDIO ONLY</div></div><aside class="projection-console"><b>收到：不明年代</b><div class="projection-log"><p>“喂？我听得见你那边的电流。你那儿是2000年？”</p><p>“我手里登记表写2442。先别说穿越，机器标签也会贴错。只记能确定的。”</p><p>“下面三句话，哪句是你亲眼能从镜像里核对的？”</p></div><div class="record-choices"><button data-record="fact">00:17:03，拨号记录连续三次断线。</button><button data-record="guess">这台投射仪来自2442年。</button><button data-record="guess">耳鱼改变了雾港的过去。</button></div><p class="projection-status" id="record-status">链路剩余 17 秒。</p></aside></div>`;
  $$('[data-record]',w).forEach(choice=>choice.onclick=()=>{if(choice.dataset.record!=="fact"){$("#record-status",w).textContent="“这是推断，不是记录。再看一眼。”";return}renderProjectionEnding(w)});
}
function renderProjectionEnding(w){
  state.projection=true;save();
  $(".window-body",w).innerHTML=`<div class="projection-ending"><div><small>SIDE A / LINK CLOSED</small><h1>雾港投影</h1><p>“对。先保住这一句。”耳鱼说。</p><p>载波断开前，屏幕跳出一行新目录：</p><pre>E:\\PROJECTION\\0xA\\0017\\</pre><p>目录日期显示为 2442 年，文件大小仍在增加。</p><a href="night-shift/">进入《苍绿之眼：和弦夜班》</a><br><a href="https://kuthorx.github.io/posts/story_of_cloak/" target="_blank" rel="noopener">阅读《苍绿之眼》故事线</a></div></div>`;
}
