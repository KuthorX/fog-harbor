...
function openProjector(){
  ...
  $("#projection-expose",w).onclick=()=>{if(Math.abs(+wave.value-546.1)>1){$("#projection-status",w).textContent="画面偏色。卡槽没有响应。";return}renderProjectionCarrier(w)};
}
function renderProjectionCarrier(w){
  ...
  $("#carrier-form",w).onsubmit=e=>{e.preventDefault();if($("#carrier-frequency",w).value.trim()!=="4597"){$("#carrier-status",w).textContent="只有电流声。";return}renderProjectionRecord(w)};
}
