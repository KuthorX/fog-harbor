...
function decision6(){return `<form class="decision field-grid" id="decision6"><fieldset><legend>重建投射卡</legend><label>来源年份 <input name="origin" inputmode="numeric" maxlength="4" autocomplete="off"></label><label>载体记录 <input name="carrier" maxlength="8" autocomplete="off"></label><label>目标编号 <input name="target" maxlength="8" autocomplete="off"></label><label>临时编号 <input name="alias" maxlength="8" autocomplete="off"></label></fieldset><fieldset><legend>字段处理</legend>${radio("action",[["separate","四个字段分别保留"],["merge","合并成同一人物"],["target","用目标覆盖其他字段"]])}</fieldset><button class="primary">重写投射卡</button><p class="feedback" id="feedback6"></p></form>`}
...
function decision8(){const ready=state.muxSeen.length===3;return `<form class="decision" id="decision8"><fieldset><legend>三路合并后的意思</legend>${radio("meaning",[["conditional","同意开店，但拒绝把血样、投射和劳动混成“免费”"],["free","无条件提供免费服务"],["refuse","拒绝在任何时间线合作"]])}</fieldset><fieldset><legend>写入合同</legend>${radio("action",[["separate","分别记录血样来源待查、投射成本和饭店劳动"],["free","统一记为研究所免费福利"],["delete","只保留第一路“成交”"]])}</fieldset><button class="primary" ${ready?"":"disabled"}>合并三路记录</button><p class="feedback" id="feedback8">${ready?"":"通信原则要求至少三种方式；当前还没读完全部附件。"}</p></form>`}
...
function decision10(){const ready=state.masterSeen.length===3;return `<form class="decision" id="decision10"><fieldset><legend>母带如何交付？</legend>${radio("mix",[["open","保留十一轨及其状态，不用相邻声部补写未知内容"],["complete","静音未知轨，导出“完整答案”"],["merge","把同编号、同颜色、同关系的声部自动合并"]])}</fieldset><fieldset><legend>位面 B 坐标</legend>${radio("route",[["hold","只保存坐标；目标未陈述前不执行投射"],["launch","立即投射到凯特·沃尔"],["replace","继续用西宫代替目标"]])}</fieldset><button class="primary" ${ready?"":"disabled"}>交付开放母带</button><p class="feedback" id="feedback10">${ready?"":"母带交付前，三份附件都要进监听记录。"}</p></form>`}
